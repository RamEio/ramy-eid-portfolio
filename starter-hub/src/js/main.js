// Agent content for different platforms
const agents = {
    chatgpt: `# Project Starter Agent for ChatGPT
...
*Please describe your project idea, and I'll guide you through creating a complete, deployable system in under 10 minutes!*`,
    ...
    gemini: `# Project Starter Agent for Gemini Pro
...
*Please describe your project idea, and I'll guide you through creating a complete, deployable system in under 10 minutes!*`,
    ...
    claude: `# Project Starter Agent for Claude
...
// Copy agent function
function copyAgent(platform) {
    const agentText = agents[platform];
    
    // Use the modern clipboard API
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(agentText).then(() => {
            showCopyNotification(`Agent copied for ${platform.toUpperCase()}!`);
        }).catch(err => {
            console.error('Failed to copy: ', err);
            fallbackCopyTextToClipboard(agentText);
        });
    } else {
...
function fallbackCopyTextToClipboard(text) {
    ...
    try {
        ...
        if (successful) {
            showCopyNotification("Agent copied to clipboard!");
        } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const animateElements = document.querySelectorAll('.feature, .example-card, .testimonial, .step');
